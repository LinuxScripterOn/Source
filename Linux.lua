...
function Linux:SaveConfig(key, value, shouldSave)
    if not shouldSave then return end
    local success, err = pcall(function()
        if not isfolder("LinuxUI") then
            makefolder("LinuxUI")
        end
        local configFile = "LinuxUI/config.json"
        local configData = {}
        if isfile(configFile) then
            configData = game:GetService("HttpService"):JSONDecode(readfile(configFile))
        end
        configData[key] = value
        writefile(configFile, game:GetService("HttpService"):JSONEncode(configData))
    end)
    if not success then
        self:Notify({
            Title = "Save Error",
...
function Linux:LoadConfig(key, default, shouldSave)
    if not shouldSave then return default end
    local success, value = pcall(function()
        local configFile = "LinuxUI/config.json"
        if isfile(configFile) then
            local configData = game:GetService("HttpService"):JSONDecode(readfile(configFile))
...
function Linux.Create(config)
    ...
    local TabsBar = Linux.Instance("Frame", {
        ...
        Size = UDim2.new(0, config.TabWidth or 110, 1, -30),
        ZIndex = 2,
        BorderSizePixel = 0
    })
    ...
    local Content = Linux.Instance("Frame", {
        Parent = Main,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, config.TabWidth or 110, 0, 30),
        Size = UDim2.new(1, -(config.TabWidth or 110), 1, -30),
        ZIndex = 1,
        BorderSizePixel = 0
    })
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Button(config)
            ...
            local clickColor = Color3.fromRGB(70, 70, 70)
        ...
        function TabElements.Toggle(config)
            ...
            local State = Linux:LoadConfig("Toggle_" .. config.Name, config.Default or false, shouldSave)
            ...
            local function UpdateToggle()
                ...
                if State then
                    ...
                    TweenService:Create(ToggleBox, tween, {BackgroundColor3 = Color3.fromRGB(80, 80, 80)}):Play()
                else
                    CheckIcon.Visible = false
                    TweenService:Create(ToggleBox, tween, {BackgroundColor3 = Color3.fromRGB(60, 60, 60)}):Play()
                end
                task.wait(0.2)
                isToggling = false
                Linux:SaveConfig("Toggle_" .. config.Name, State, shouldSave)
            end
        ...
        function TabElements.Dropdown(config)
            ...
            local Selected = Linux.Instance("TextLabel", {
                ...
                Font = Enum.Font.Gotham,
                Text = Linux:LoadConfig("Dropdown_" .. config.Name, config.Default or (config.Options and config.Options[1]) or "None", shouldSave),
                TextColor3 = Color3.fromRGB(255, 255, 255),
                TextSize = 14,
            ...
            local IsOpen = false
            local SelectedValue = Linux:LoadConfig("Dropdown_" .. config.Name, config.Default or (Options[1] or "None"), shouldSave)
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            ...
                            Linux:SaveConfig("Dropdown_" .. config.Name, SelectedValue, shouldSave)
        ...
        function TabElements.Slider(config)
            ...
            local Default = Linux:LoadConfig("Slider_" .. config.Name, config.Default or Min, shouldSave)
            local Value = Default
            ...
            local function UpdateSlider(pos)
                ...
                AnimateValueLabel()
                Linux:SaveConfig("Slider_" .. config.Name, Value, shouldSave)
                spawn(function() Linux:SafeCallback(config.Callback, Value) end)
            end
        ...
        function TabElements.Input(config)
            ...
            local TextBox = Linux.Instance("TextBox", {
                ...
                Font = Enum.Font.Gotham,
                Text = Linux:LoadConfig("Input_" .. config.Name, config.Default or "", shouldSave),
            ...
            local function UpdateInput()
                CheckTextBounds()
                Linux:SaveConfig("Input_" .. config.Name, TextBox.Text, shouldSave)
